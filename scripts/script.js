...
(async function () {
    // Locate the plane in the Scene
    const [plane, paper1, paper2, paper3, rectangle1, rectangle2, rectangle3, rectangle4] = await Promise.all([
    ...
    function hasTouchBottom(item, index) {
        ...
        const r4distance = Reactive.distance(rectangle4.transform.y, item.transform.y);
        Diagnostics.watch('r4 Distance =>', r4distance);
        r4distance.monitor().subscribe((e) => {
            if (e.newValue <= 0) {
                reset(item);
    ...
    function setImageSway(item) {
        ...
        const timeDriver = Animation.timeDriver(timeDriverParameters);
        const quadraticSampler = Animation.samplers.easeInOutQuad(-item.transform.x.pinLastValue(), 100);
        const translationAnimation = Animation.animate(
            timeDriver,
            quadraticSampler
        );
    ...
    function reset(item) {
        ...
        const rebornPosition = [rectangle3, rectangle2, rectangle1]
        // const ScreenY = Patches.getScalarValue('ScreenY').pinLastValue();
        const planeInitialYPosition = rebornPosition[randomNum].transform.y.pinLastValue();
        const planeEndYPosition = rectangle4.transform.y.pinLastValue();
        const timeDriver = Animation.timeDriver({
            durationMilliseconds: 10,
            loopCount: 1,
...
>>>>>>> 3e3cba67fa7557849f6de906096a573aaed216ed
